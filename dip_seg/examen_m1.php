...
if (count($resultadoEF)>=3 || $resultadoEF[0]["Calificacion_Final"] >=70) {
    header("Location:modulo2.php?p=1");
}
...
function myGreeting() {
    window.location = 'modulo1.php?p=1';
}
</script>
        ...
        <div class="col-sm-offset-2 col-sm-10">
            <p class="pull-right">
                ...
                <button class="btn btn-default" type="button"
                    onClick="document.location.href='modulo1.php?p=1'">Regresar</button>
                <?php if(!$_POST){?>
...
function valida() {
    ...
    if (!confirm(
            "¿Estas seguro de enviar la evaluación final del Módulo 1?\n\n-Haz clic en el botón \"Cancelar\" si deseas realizar algún cambio en las respuestas."
            )) {
        return false;
    } else {
        form.action = "examenm1.php";
        form.method = "post";
        form.submit();
    }
