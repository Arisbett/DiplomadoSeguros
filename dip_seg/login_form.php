...
//echo "c.nombreUniv_Inst,c.imagen,a.idGrupo,a.idAlumno,upper(a.nombre) as nombre,upper(a.apaterno) as apaterno,upper(a.amaterno) as amaterno from alumno a,grupo c,expedientes d where  c.idGrupo=a.idGrupo AND a.idAlumno=d.id_alumno and d.autorizado='1' and d.id_generacion=$id_generacion and a.autorizado='1' AND TRIM(a.correo)='$usuario' and TRIM(a.pass)='$pswd'";
    $info_usuario = select("upper(c.nombre) as institucion, a.id_alumno,a.id_grupo,a.id_alumno,upper(a.nombre) as nombre,upper(a.apaterno) as apaterno,upper(a.amaterno) as amaterno,g.registro_inicio, g.registro_fin, g.fin_ds",
     "alumno a,grupo c,generaciones g", " c.id_grupo=a.id_grupo and a.id_generacion=$id_generacion and a.autorizacion='1' AND a.id_generacion=$id_generacion AND TRIM(a.correo)='$usuario' and TRIM(a.pass)='$pswd'", "");
