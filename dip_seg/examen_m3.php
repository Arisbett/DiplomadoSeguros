...
<!-- termina cronometro y cambio de parametro-->
            ...
            <form role="form" name="examen">
                ...
                <?php
                    if(!$nuevowhere){ 
                    $comando = Conexion::conectar()->query("SELECT * FROM actividades_preguntas WHERE id_actividad='21' ORDER BY rand() LIMIT 15");
                    } else { 
                    ...
                    foreach($resultado AS $row){ 
                        ...
                        $comando1 = Conexion::conectar()->query("SELECT id_actividad_opciones,id_pregunta, opcion FROM actividades_opciones WHERE id_actividad='21' AND id_pregunta=$row[id_actividad_preguntas]");
                        $comando1->execute();
...
//alert(document.getElementById("r1_1")+"*"+document.getElementById("r1_2")+"*"+document.getElementById("r1_3"))
    ...
    if (!confirm ("¿Estas seguro de enviar la evaluación final del Módulo 3?\n\n-Haz clic en el botón \"Cancelar\" si deseas realizar algún cambio en las respuestas.")){
		return false;
	} else {
        form.action="examenm3.php"; 
   form.method="post";
   form.submit();
    }
