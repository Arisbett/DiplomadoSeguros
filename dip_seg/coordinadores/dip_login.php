...
if($usuario && $pswd){
    ...
    $info_usuario = select("id_coordinadores,id_grupo,nombre,password", "coordinadores", "activo='1' AND usuario='$usuario' ", "", ""); 
    $pass_bd = $info_usuario[0]["password"];
    $id_coordinador=$info_usuario[0]["id_coordinadores"];
    ...
    //$id_tipo_coordinador = $info_usuario[0]["id_tipo_coordina"];
     ...
     if(verifica_pass($pswd,$pass_bd)==1)
                $id_usuario = $info_usuario[0]["id_coordinadores"];
    else
                $id_usuario = 0;

    if($id_usuario>0){
    
        $grupo = select("nombre", "grupo", "id_grupo=$id_grupo", "", "");
        $nombre_grupo = $grupo[0]["nombre"];

        session_start();
        $_SESSION["autentificado"] 	= "SI";
        $_SESSION["id_coordinadores"] = $id_coordinador;
        $_SESSION["nombre"] 		= $nombre;
        //$_SESSION["tipo_coordinador"] = $id_tipo_coordinador;
        $_SESSION["id_grupo"]   	  = $id_grupo;
        $_SESSION["nombre_grupo"]       = $nombre_grupo;
        $_SESSION["fecha1_registro"]    = "2023-02-15"; //"22-08-2022";
        $_SESSION["fecha2_registro"]    = "2024-11-31";
        $_SESSION["id_generacion"]  =   "43";
