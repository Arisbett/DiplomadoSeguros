...
function select($campos, $tablas, $where="1", $order="1", $limit="") {
	...
	$tmp   = array("if(","IF(","delete ","DELETE ","insert ","INSERT ","drop ","DROP ","EXTRACTVALUE","extractvalue","/**//**/","/**/","_SCHEMA",'~',"ALTER ","alter ","table","TABLE","ETL(","etl(","ETL (","etl (","FLOOR","floor","RAND(","RAND (","rand(","rand (","ROW(","row(");
     $entra=1;
     for($i=0;$i<count($tmp);$i++){
            $pos = strripos($sql_query, $tmp[$i]);
            if(!$pos==false){ echo '<br>'.$pos.'*'.$tmp[$i];
                $entra=0;
                $sql_query="";
                break;
            }
