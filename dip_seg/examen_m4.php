...
$comandoEF = $con->query("SELECT idExamen, Calificacion_Final from examen_m4 WHERE idAlumno=$idAl");
$comandoEF->execute();
$resultadoEF = $comandoEF->fetchAll(PDO::FETCH_ASSOC);

if (count($resultadoEF)>=3 || $resultadoEF[0]["Calificacion_Final"] >=70) {
    header("Location:modulo4.php?p=1");
}
...
<!-- termina cronometro y cambio de parametro-->
            ...
            <form role="form" name="examen">
                ...
                <?php
                    if(!$nuevowhere){ 
                    $comando = Conexion::conectar()->query("SELECT * FROM actividades_preguntas WHERE id_actividad='25' ORDER BY rand() LIMIT 15");
                    } else { 
                    ...
                    foreach($resultado AS $row){ 
                        ...
                        $comando1 = Conexion::conectar()->query("SELECT id_actividad_opciones,id_pregunta, opcion FROM actividades_opciones WHERE id_actividad='25' AND id_pregunta=$row[id_actividad_preguntas]");
                        $comando1->execute();
	...
	<p class="pull-right">    
        ...
        <button class="btn btn-default" type="button" onClick="document.location.href='modulo4.php'">Regresar</button>
        <?php if(!$_POST){?>
...
//alert(document.getElementById("r1_1")+"*"+document.getElementById("r1_2")+"*"+document.getElementById("r1_3"))
    ...
    if (!confirm ("¿Estas seguro de enviar la evaluación final del Módulo 4?\n\n-Haz clic en el botón \"Cancelar\" si deseas realizar algún cambio en las respuestas.")){
		return false;
	} else {
        form.action="examenm4.php"; 
   form.method="post";
   form.submit();
    }
